          ...
          .about-sidebar
            ...
            ul
              li
                ...
                v-chip.ml-2(size='small', color='green-lighten-5') {{sponsors.length}}
              li
                a(href='#developers') - Developers
                v-chip.ml-2(size='small', color='orange-lighten-5') {{developers.length}}
              li
                a(href='#translators') - Translators
                v-chip.ml-2(size='small', color='blue-lighten-5') {{translators.length}}
              li
                a(href='#specialthanks') - Special Thanks
                v-chip.ml-2(size='small', color='grey-lighten-3') {{specialthanks.length}}
            v-divider.my-4
            h2 Become a Sponsor
            v-btn(color='pink', dark, large, block, to='/donate') Donate
            .text-caption.mt-3.text-center Various platforms are supported, for both monthly and one-time donations.
            v-divider.my-4
            carbon
        v-col(cols='12', md='8', xl='9')
          .about-list
            ...
            .about-list-loading(v-if='loading')
              v-progress-circular(color='primary', width='2', size='24', indeterminate)
              span Fetching list of sponsors...
            v-container.pa-0.mb-4
              v-row
                v-col(cols='12', xl='6', v-for='(sponsor, idx) of sponsors', :key='sponsor.id')
                  .about-list-item.animated.fadeInUp(:class='`wait-p` + idx + `s`')
                    ...
                    v-avatar(v-else, color='green-darken-1', size='68', tile)
                      .text-h5.text-white {{getInitials(sponsor.name)}}
            ...
            .about-list-loading(v-if='loading')
              v-progress-circular(color='orange', width='2', size='24', indeterminate)
              span Fetching list of developers...
            v-container.pa-0.mb-4
              v-row
                v-col(cols='12', md='6', xl='4', v-for='developer of developers', :key='developer.id')
                  .about-list-item
                    ...
                    v-avatar(v-else, color='green-darken-1', size='48', tile)
                      .text-h5.text-white {{getInitials(developer.name)}}
                    v-divider.mx-3(vertical)
                    .about-list-item-text
                      strong(v-html='developer.name')
                      .text-orange.text-caption {{developer.extraInfo}} commits
                    v-spacer
            ...
            v-container.pa-0.mb-4
              v-row
                v-col(cols='12', md='6', xl='4', v-for='translator of translators', :key='translator.id')
                  .about-list-item
                    v-avatar(color='blue', size='48', tile)
                      .text-h6.text-white {{getInitials(translator.name)}}
                    v-divider.mx-3(vertical)
                    .about-list-item-text
                      strong(v-html='translator.name')
                      .text-blue.text-caption(v-html='translator.extraInfo')
...
const sponsors = computed(() => _.get(result.value, 'sponsors.backers', []))
const translators = computed(() => _.get(result.value, 'sponsors.translators', []))
const developers = computed(() => _.get(result.value, 'sponsors.developers', []))
